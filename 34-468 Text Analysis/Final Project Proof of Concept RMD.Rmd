...
# --> TS songs folder 
```{r}
# reading in song lyrics txt files and form a corpus
...
ts_csv2 <- read.csv("../final_project/Taylor_Swift.csv") # no albums column
...
make_key = function(ts, bey){
 ...
 bey_dfm <- bey %>%
  tokens_compound(pattern = phrase(multiword_expressions)) %>%
  dfm()
 return(keyness_table(ts_dfm, bey_dfm))
}
  

overall_key = make_key(ts, bey)
